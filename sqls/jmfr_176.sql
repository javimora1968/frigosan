alter table dba.fr_ganados  add
       id_doc_factura_basc char(10) null,
       id_factura_basc numeric(18,0) null;

update dbo.pbcattbl set
       pbd_fhgt = -10, pbd_fwgt = 400, pbd_fitl = 'N', pbd_funl = 'N',
       pbd_fchr = 0, pbd_fptc = 34, pbd_ffce = 'Arial',
       pbh_fhgt = -10, pbh_fwgt = 400 , pbh_fitl = 'N', pbh_funl = 'N',
       pbh_fchr = 0, pbh_fptc = 34, pbh_ffce = 'Arial',
       pbl_fhgt = -10, pbl_fwgt = 400 , pbl_fitl = 'N', pbl_funl = 'N',
       pbl_fchr = 0, pbl_fptc = 34, pbl_ffce = 'Arial',
       pbt_cmnt = ' '
        where pbt_tid = object_id('dba.fr_ganados');

update dbo.pbcatcol  set pbc_cnam = 'id_doc_factura_basc', pbc_labl = 'Id Doc Factura Basc:',
       pbc_lpos = 0 , pbc_hdr = 'Id Doc Factura Basc', pbc_hpos = 0 ,
       pbc_jtfy = 23 , pbc_mask = '', pbc_case = 0 ,
       pbc_hght = 65 , pbc_wdth = 238 , pbc_ptrn = '',
       pbc_bmap = 'N', pbc_init = '',
       pbc_cmnt = '',
       pbc_edit = ''
       where pbc_tid = object_id('dba.fr_ganados')  and pbc_cid = 70;

insert into dbo.pbcatcol
       (pbc_tnam, pbc_tid, pbc_ownr, pbc_cnam,
       pbc_cid, pbc_labl, pbc_lpos, pbc_hdr,
       pbc_hpos, pbc_jtfy, pbc_mask, pbc_case,
       pbc_hght, pbc_wdth, pbc_ptrn, pbc_bmap,
       pbc_init, pbc_cmnt, pbc_edit, pbc_tag)
        values  ( 'fr_ganados',
        object_id('dba.fr_ganados'),
        'dba',  'id_factura_basc', 71,
        'Id Factura Basc:', 0 ,
        'Id Factura Basc', 0,
       24 , '[General]',
       0 , 65 , 421 ,
        '', 'N',
        '',
        '',
        '', '');

